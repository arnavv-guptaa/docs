...
<Tabs>
  <Tab title="Operating leases">
    ...
    Under IFRS 16 and ASC 842 no adjustment is normally required, because reported EBIT already excludes cash rent and already includes depreciation of the right-of-use asset. The interest element sits below EBIT, where it belongs.

    An adjustment is needed only for a period or a jurisdiction reported on the older operating-lease basis, where rent sits inside EBIT and no lease asset is recognized:

    `Lease-adjusted EBIT = reported EBIT + rent expense − imputed depreciation of the capitalized lease`

    Applying that formula to statements already prepared under the modern standard adds back rent that is not in EBIT and deducts lease depreciation twice.
  </Tab>
